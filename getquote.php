...
// Fetch plot statuses from the database
$query = "SELECT ProductName, Status FROM products";
$stmt = $pdo->prepare($query);
$stmt->execute();
$plots = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Create an array to map ProductName to Status
$plotStatus = [];
foreach ($plots as $plot) {
    $plotStatus[$plot['ProductName']] = $plot['Status'];
}

// Function to get background color based on status
function getBackgroundColor($plotName, $plotStatus)
{
    return (isset($plotStatus[$plotName]) && $plotStatus[$plotName] === 'booked') ? 'red' : 'LightGreen';
}
?>
