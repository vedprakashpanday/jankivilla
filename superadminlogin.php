...
function sendOtpEmail($pdo, $sponsorid, $sponsor_name, $otp)
{
    try {
        ...
        $mail->setFrom('dharamkumar211975@gmail.com', 'Dharam');
        $mail->addAddress('dharamkumar211975@gmail.com', $sponsor_name);
        $mail->isHTML(true);
...
if (isset($_POST['btn_login'])) {
    ...
    $sponsorpass = $_POST['super_admin_pass'];

    // Check credentials with status = 'active'
    $select = $pdo->prepare("SELECT * FROM tbl_hire WHERE sponsor_id = ? AND super_admin_pass = ? AND status = 'active'");
    $select->execute([$sponsorid, $sponsorpass]);
    $row = $select->fetch(PDO::FETCH_ASSOC);

    if ($row && $row['sponsor_id'] === $sponsorid && $row['super_admin_pass'] === $sponsorpass && $row['status'] === 'active') {
        // Generate OTP
        ...
        $_SESSION['temp_super_admin_pass'] = $row['super_admin_pass'];
...
if (isset($_GET['resend_otp']) && $_GET['resend_otp'] == 1) {
    ...
    if (!$sponsorid || !$sponsor_name) {
        header("Location: superadminlogin.php"); // Redirect to login page if session expired
        exit();
    }
...
if (isset($_POST['verify_otp'])) {
    ...
    if (!$sponsorid) {
        header("Location: superadminlogin.php"); // Redirect to login if session expired
        exit();
    }
...
<body>
    <div class="otp-container">
        ...
        $_SESSION['sponsor_pass'] = $row['super_admin_pass'];
        ...
        if (isset($_POST['remember_me']) && $_POST['remember_me'] == 'on') {
            setcookie('sponsor_login', json_encode(['sponsorid' => $row['sponsor_id'], 'sponsorpass' => $row['super_admin_pass']]), time() + 2592000, "/");
        } else {
...
<body>
    <div class="success-container">
        ...
        header("Refresh:1; url=UI/superadmin/dashboard.php");
        exit();
    } else {
            ...
            <form action="" method="post">
                <div class="login-form">
                    <div class="container col-md-4">
                        ...
                        <input type="password" class="form-control" placeholder="Password" name="super_admin_pass" required>
