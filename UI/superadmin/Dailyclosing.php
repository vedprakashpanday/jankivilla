...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <div class="wrapper">
                                    ...
                                    foreach ($closed_history as $history) {
                                        ...
                                        if ($payment_details) {
                                            foreach ($payment_details as $productname => $details) {
                                                ...
                                                // Mark this product as already processed (achieved 25% and counted)
        ...
        ";
                                    ...
                                    foreach ($cumulative_tracking as $key => $tracking_data) {
                                        if ($tracking_data['is_eligible']) {
                                            ...
                                            if (!$is_already_processed) {
                                                ...
                                                // Include ALL payments if 25% was achieved within the current date range
                                                ...
                                                // Include only NEW payments after the last counted date and within date range
                                                ...
                                                foreach ($tracking_data['payments'] as $payment) {
                                                    ...
                                                    // Include payment if it's after the last counted date AND within current date range
                                            ...
                                            // Also include products that achieved 25% BEFORE the current period but have new payments in current period
                                            if (!$is_already_processed && $product_25_date < $from_date) {
                                                // Product achieved 25% before current period, include only payments within current date range
                                                foreach ($tracking_data['payments'] as $payment) {
                                                    $payment_date = $payment['created_date'];
                                                    $payment_amount = floatval($payment['payamount']);
                                            ...
                                            // Add to period eligible amounts if there are actual eligible payments
                                    ...
                                    // Step 5: Get all members from tbl_regist
                                    $members_query = "
            SELECT 
        ...
        ";
            ...
            ";
                                        ...
                                        // FIXED: Calculate direct commission based on member's self business (direct_amount)
                    ...
                    ";
                                    ...
                                    // Step 9: FIXED - Calculate total commission AFTER all level commissions are calculated
                                    foreach ($commission_report as $member_id => &$member) {
        ...
        • Products already processed in previous closings: Only payments after last closing date AND within current date range are included</div>";
            ...
            ";
                                        ...
                                        foreach ($sorted_report as $report) {
                                            // Prepare payment_details JSON properly - Include ALL payments that are displayed
                                            $payment_json = [];
                                            if (!empty($report['payment_details'])) {
                                                foreach ($report['payment_details'] as $productname => $details) {
                                            ...
                                            // FIXED: Ensure total_commission is always the sum of direct_commission + level_commission
                                            ...
                                            $stmt->execute([
                                                ...
                                                ':total_commission' => $total_commission_to_save,  // FIXED: Using calculated total
                                                ':from_date' => $from_date,
                                                ':to_date' => $to_date,
