    ...
    if ($data['payment_type'] == 'allot') {
        ...
        foreach ($payments as $payment) {
            ...
            if ($payment['flag']==1 && $payment['enrollment_charge']>0 && ($payment['admission_charge']>=0)) {
                ...
                $debug[] = ['stage'=>'CASE1', 'count'=>$count, 'allot'=>$allot,'due'=>$due];
            }
                ...
                $debug[] = ['stage'=>'CASE2','count'=>$count,'allot'=>$allot,'due'=>$due];
            }
                ...
                $pdo->prepare("UPDATE receiveallpayment SET flag = 0,enrollment_charge = ? WHERE invoice_id = ?")
                    ->execute([15000,$data['invoice_id']]);
                ...
                $debug[] = ['stage'=>'CASE_DEFAULT','count'=>$count,'allot'=>$allot,'due'=>$due];
            }

            $loop++;
        }

        $debug[] = ['stage' => 'ALLOT_END', 'count'=> $count, 'allot'=>$allot, 'due'=>$due];
    }
