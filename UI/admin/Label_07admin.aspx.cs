...
public partial class UI_admin_Label_07 : System.Web.UI.Page
{
    ...
    public void BindDetail()
    {
        ...
        SqlCommand cmd = new SqlCommand("select ID,MemberID,SponsorID,MemberName,MobileNo ,JoiningDate,2 as Amount  from Member_Details    where SponsorID in (select MemberID from Member_Details where SponsorID in (select MemberID from Member_Details where SponsorID in (select MemberID from Member_Details where SponsorID in (select MemberID from Member_Details where SponsorID in (select MemberID from Member_Details where SponsorID in (select MemberID from Member_Details where SponsorID='" + MemID + "' and  Id>" + id + " and  Status='Active' )))))) and Status='Active' ", con);
