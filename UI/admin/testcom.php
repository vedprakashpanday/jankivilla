...
if ($action === 'submit_payment') {
    ...
    if ($data['payment_type'] == 'enroll') {
        ...
        // $enroll=$data['payamount'];
        // $count=$data['payamount'];
...
$count=6;
    ...
    if ($data['payment_type'] == 'allot') { 

          
         $stmt526 = $pdo->prepare("SELECT flag,admission_charge,enrollment_charge,payamount FROM receiveallpayment WHERE invoice_id = ? ");
            ...
            foreach($payments as $payment)
            {
                if($payment['flag']==1 && $payment['enrollment_charge']>0 &&($payment['admission_charge']>=0))
                {
                    ...
                    $due=$data['due_amount'] -($data['payamount']+15000);

                    $update = $pdo->prepare("UPDATE receiveallpayment SET flag = 0 WHERE invoice_id = ?");
               ...
               else if($payment['flag']==2 && $payment['enrollment_charge']>0 && $payment['admission_charge']>0 && $payment['payamount']>0 )
                {
                    $count=3;
                    $allot=$data['payamount'];
                    $due=$data['due_amount'] -$data['payamount'];
                ...
                else if($payment['flag']==0 && $payment['admission_charge']>0 && $payment['enrollment_charge']==0  && $payment['payamount']==0)
                {
                    $count=4;
                    $allot=$data['payamount']-15000;
                    $due=$data['due_amount'] -$data['payamount'];

                     $update = $pdo->prepare("UPDATE receiveallpayment SET flag = 0,enrollment_charge = ? WHERE invoice_id = ?");
                    $update->execute([15000,$data['invoice_id']]);
                    $loop++;
                }
                else
                    {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO receiveallpayment (
                ...
                neft_payment, rtgs_payment, created_date, admission_charge, enrollment_charge
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $data['invoice_id'],
...
if(!empty($admission)&&!empty($enroll))
        ...
        if(!empty($enroll)&& empty($admission))
        {
        ...
        if (!sendInvoiceEmail($pdo, $data['invoice_id'], $data['customer_name'], $data ,$count,$loop)) {
            ...
            echo json_encode(['success' => true, 'due_amount' => $data['due_amount'], 'message' => 'Payment recorded and invoice emailed to customer','data' => $data,'count'=>$count,'loop'=>$loop]);
        }
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
