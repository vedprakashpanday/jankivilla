...
foreach ($level8_members as $level8_member) {
    $level9_stmt = $pdo->prepare("SELECT * FROM tbl_regist WHERE sponsor_id = :member_id");
...
// Output level 10 members

$level10_members = array();
foreach ($level9_members as $level9_member) {
    $level10_stmt = $pdo->prepare("SELECT DISTINCT r.sponsor_id,r.s_name, r.m_name, r.mem_sid, p.package, r.date_time, p.status  FROM tbl_regist r LEFT JOIN tbl_package p ON r.mem_sid = p.member_id left join tbl_acc a ON r.mem_sid = a.member_id WHERE r.sponsor_id = :member_id");
    $level10_stmt->bindParam(':member_id', $level9_member['mem_sid']);
    $level10_stmt->execute();
    $level10_members = array_merge($level10_members, $level10_stmt->fetchAll(PDO::FETCH_ASSOC));
}
foreach ($level10_members as $member) {
    // echo $member['m_name'] . $member['package'] . "<br>";
}

//output level 11 members
$level11_members = array();
foreach ($level10_members as $level10_member) {
    $level11_stmt = $pdo->prepare("SELECT * FROM tbl_regist WHERE sponsor_id = :member_id");
    $level11_stmt->bindParam(':member_id', $level10_member['mem_sid']);
    $level11_stmt->execute();
    $level11_members = array_merge($level11_members, $level11_stmt->fetchAll(PDO::FETCH_ASSOC));
}
foreach ($level11_members as $member) {
}

// Output level 12 members
$level12_members = array();
foreach ($level11_members as $level11_member) {
    $level12_stmt = $pdo->prepare("SELECT * FROM tbl_regist WHERE sponsor_id = :member_id");
    $level12_stmt->bindParam(':member_id', $level11_member['mem_sid']);
    $level12_stmt->execute();
    $level12_members = array_merge($level12_members, $level12_stmt->fetchAll(PDO::FETCH_ASSOC));
}
foreach ($level12_members as $member) {
}
                ...
                <div class="main-panel">
                    <div class="content-wrapper" style="background:unset!important;">
                        ...
                        <div class="sec-box">
                            ...
                            <header>
                                <h2 class="heading" style="margin:1rem;">Founder Member (F.M.)</h2>
                            </header>
                        </div>
                            ...
                            <div id="" style="overflow:auto;width:100%">
                                <table id='salesTable' class="table table-striped">
                                    ...
                                    <tbody>
                                          ...
                                          $designation = 'Founder Member (F.M.)';
