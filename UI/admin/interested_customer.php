...
<form method="post">
    ...
    <div class="mb-3 col-md-6">
        ...
        <select name="telecaller" class="form-control" required>
  ...
  $stmt = $pdo->prepare("
    ...
    FROM adm_regist 
    WHERE designation IN (:desig1, :desig2)
");

$stmt->execute([
    'desig1' => 'Telecaller',
    'desig2' => 'Senior Relationship Manager(SRM)'
]);

$telecallers = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $selectedTelecaller = $edit_data['assigned_telecaller'] ?? '';

    foreach ($telecallers as $row):
        $selected = ($row['member_id'] == $selectedTelecaller) ? 'selected' : '';
    ?>
        <option value="<?= htmlspecialchars($row['member_id']) ?>" <?= $selected ?>>
                 ...
                 <div class="col-md-2">
                    <button class="btn btn-primary w-100 mt-3" onclick="printTable()">
                        🖨 Today Print
                           ...
                           <hr>
                            <h4 class="my-3 py-3">Interested Customer List</h4>
                            <hr>
...
<table class="table table-bordered table-striped" style="min-width: 1600px;" id="employeeTable">
    ...
    <?php
    $customers = $pdo->query(
        "SELECT * FROM interested_customer ORDER BY id DESC"
