...
function sendOtpEmail($pdo, $user_id, $user_name, $user_email, $otp)
{
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'dharamkumar211975@gmail.com'; // Replace with your email
        $mail->Password = 'luqa nzkd ffjj lehy'; // Replace with your app password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;
        $mail->setFrom('dharamkumar211975@gmail.com', 'Dharam Kumar');
        $mail->addAddress('dharamkumar211975@gmail.com', 'Dharam Kumar'); // Replace with user's email
        $mail->isHTML(true);
...
if (isset($_POST['btn_login'])) {
    ...
    // Check tbl_account_login first
    $select = $pdo->prepare("SELECT * FROM tbl_account_login WHERE sponsor_id = ? AND sponsor_pass = ?");
    $select->execute([$user_id, $password]);
    $row = $select->fetch(PDO::FETCH_ASSOC);

    if ($row) {
        // Account found in tbl_account_login
        $otp = rand(100000, 999999); // 6-digit OTP
        $otp_created_at = date('Y-m-d H:i:s');
        $update = $pdo->prepare("UPDATE tbl_account_login SET otp = ?, otp_created_at = ? WHERE sponsor_id = ?");
        $update->execute([$otp, $otp_created_at, $user_id]);

        // Store temporary session data
        $_SESSION['temp_user_id'] = $row['sponsor_id'];
        $_SESSION['temp_user_name'] = $row['s_name'];
        $_SESSION['temp_user_email'] = 'dharamkumar211975@gmail.com'; // Replace with actual email if available
        $_SESSION['temp_status'] = $row['status'];
        $_SESSION['temp_user_type'] = 'account_login';
        ...
        $select = $pdo->prepare("SELECT * FROM employees WHERE emp_id = ? AND password = ?");
        $select->execute([$user_id, $password]);
        $employee = $select->fetch(PDO::FETCH_ASSOC);

        if ($employee) {
            // Account found in employees
            $otp = rand(100000, 999999); // 6-digit OTP
            $otp_created_at = date('Y-m-d H:i:s');
...
if (isset($_GET['clear_otp']) && $_GET['clear_otp'] == 1) {
    ...
    if ($user_id && $user_id === ($_SESSION['temp_user_id'] ?? '')) {
        if ($user_type === 'account_login') {
            $update = $pdo->prepare("UPDATE tbl_account_login SET otp = NULL, otp_created_at = NULL WHERE sponsor_id = ?");
        } else {
...
if (isset($_POST['verify_otp'])) {
    ...
    // Check OTP
    if ($user_type === 'account_login') {
        $select = $pdo->prepare("SELECT * FROM tbl_account_login WHERE sponsor_id = ? AND otp = ?");
        $select->execute([$user_id, $entered_otp]);
        $row = $select->fetch(PDO::FETCH_ASSOC);
        $otp_created_at = $row ? strtotime($row['otp_created_at']) : 0;
    } else {
        $select = $pdo->prepare("SELECT * FROM employees WHERE emp_id = ? AND otp = ?");
        $select->execute([$user_id, $entered_otp]);
        $row = $select->fetch(PDO::FETCH_ASSOC);
    ...
    if ($row && $row['otp'] == $entered_otp) {
        ...
        $update->execute([$user_id]);
        $_SESSION['role'] = 'account'; // For accountant panel
